...
指令 (Directives) 是带有`v-`前缀的特殊属性. 当表达式值改变时, 将其产生的连带影响响应式的作用于 DOM, 在 Vue 模板编译的时候, 会将指令系统转化成可执行的`render`函数
...
在 Vue 实例中定义`data`可以是一个对象, 也可以是一个函数. 但是在组件中定义`data`时, 只能是一个函数. Vue 组件之间是共享`data`属性的, 以对象来定义势必会对其他组件里的数据造成污染
...
`v-for`优先级要高于`v-if`, 作用于同一个元素上时会带来性能上的浪费 (先循环再进行判断), 建议在元素外层套用`template` (页面渲染不生成 dom 节点) 进行条件判断, 内层再进行循环
...
`watch`主要用来监控 vue 实例的变化, 它监控的是 data 里定义的变量或对象, 一般用于监控路由等, 比较适用于一个数据影响多个数据, 不具有缓存性, 只要属性值发生变化, 就会触发回调函数
...
双向绑定主要由 MVVM 中的 VM 决定, ViewModel 有两个主要组成部分:
...
2. 然后编译模版, 从`data`中找到动态绑定的数据并初始化视图, 这个过程发生在 Compile 中
...
生命周期通俗的讲就是一个从摇篮到坟墓的过程, 在 Vue 中就是实例从创建、初始化、编译模版、挂载 Dom -> 渲染、更新 -> 渲染、卸载等一系列过程. Vue 生命周期钩子会自动绑定`this`上下文到实例中, 因此可以访问数据, 对属性和方法进行运算
...
Vuex 适用于复杂关系的组件之间数据传递, Vuex 作用相当于一个共享变量的容器, 放到 store 中
...
- 新旧都有子节点, 调用`updateChildren`进行比对 (首尾双指针法, 逐渐向中间靠拢, 进行对比查询复用)

## 插槽(Slot)

`Slot` 可以理解为占位符, 即在组件模版中占据位置, 当使用该组件标签的时候, 组件标签里的内容就会自动填充, 作为承载分发内容的出口

插槽可以让用户拓展组件, 更好的复用及定制. 通过`slot`插槽向组件内部指定位置传递内容, 完成在不同场景的应用, 如布局组件、表格列、下拉选框、弹框显示等
...
- 路由权限: 1.初始化即挂载全部路由, 并标记相应的权限信息, 每次路由跳转前做校验 2.初始化先挂载不需要权限控制的路由(登录页、404 及其他错误页等), 如果用户强制访问则进入 404. 登录后获取到用户信息, 筛选有权限访问的路由, 在全局路由守卫里调用`addRoutes`添加路由
...
**SSR**: 在服务端对页面进行渲染, 然后返回给客户端, 客户端第一次渲染时存在 js 下载的等待时间, 服务端渲染不会存在这个问题
